...
<div class="api-header">
  <div class="api-info">
    ...
    {/if}
    <div>
      <h1>Is {api.name} Down? <span class="h1-status">Real-Time Status</span></h1>
...
<div class="community-pulse">
  {#if (data.subscriberCount || 0) > 5}
    <span class="pulse-item">
      ...
      {data.subscriberCount} developers monitoring this API
    </span>
  {:else}
    <span class="pulse-item pulse-cta">Be one of the first to monitor {api.name} — <a href="#subscribe">subscribe to alerts</a></span>
  {/if}
  {#if (data.reportCount || 0) > 0}
    <span class="pulse-item">
      <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z"/><polyline points="14 2 14 8 20 8"/></svg>
      {data.reportCount} community reports submitted
    </span>
  {/if}
</div>
